...
# WITH BOUNDARY SHAPEFILE MASKING
# ============================================================
...
OUT_DIR = f"{BASE}/model_output_Logistic"
os.makedirs(OUT_DIR, exist_ok=True)

OUT_TIF = f"{OUT_DIR}/Logistic_LSM.tif"
...
# ------------------------------------------------------------
# Feature Scaling (Important for LR Stability)
# ------------------------------------------------------------
...
plt.plot(fpr, tpr, label=f"Logistic (AUC={roc_auc_score(y_te,y_prob):.3f})")
plt.plot([0,1],[0,1],'k--')
...
with rasterio.open(OUT_TIF, "w", **profile) as dst:
    ...
    for row in tqdm(range(0, H, 256), desc="Logistic Prediction"):
...
print("✅ Logistic Regression Susceptibility Map saved:")
print(OUT_TIF)
...
plt.colorbar(label="Logistic Regression Susceptibility")
plt.title("Logistic Regression Landslide Susceptibility Map")
plt.axis("off")
plt.tight_layout()
plt.show()
